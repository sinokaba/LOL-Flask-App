...
class LolCrawler:
	...
	def init_static_data(self, reset=False):
		...
		champs_data_raw = self.api_obj.get_static_data("champions")
		print(champs_data_raw)
		champs_data = champs_data_raw["data"]
	...
	def add_champs_data(self, match_details, players, role=None, match_timeline=[], patch="7.14.1"):
		...
		for team,player in players.items():
			...
			champ_obj.add_player(acc_id, player_d["summonerName"], win, self.get_kda(par_details), oa_rating, role, self.players_visited[acc_id].currentRank)
	...
	def add_player_rank(self):
		all_players = PlayerBasic.select().order_by(PlayerBasic.region)
		#player_file = open("players_d.txt", "r")
		temp = []
		for player in all_players:
			if(player.accountId not in temp):
				print(player.rank, player.summonerId)
				temp.append(player.accountId)
		#player_file.close()

	def add_overall_table_stats(self):
		all_champ_role_stats = ChampRoleOverallStats.select().join(ChampRankStats).join(StatsBase).order_by(StatsBase.region)
		data = {}
...
if __name__ == "__main__":
	lol_crawler = LolCrawler()
	lol_crawler.aggregate_rank_data("gold", 10, "NA")
	...
	lol_crawler.aggregate_rank_data("platinum", 500, "NA")
